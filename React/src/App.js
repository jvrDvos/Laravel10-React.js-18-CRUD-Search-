...
const App = () => {
  ...
  //  Show Product
    const fetchProduct = useCallback ( async () => {
        ...
        const response = await axios.get(`/Product?name=${searchQuery.name}`);
  ...
  const editProduct = async (e, id) => {
    ...
    if (nameInput && priceInput && descriptionInput && stockInput) {
      ...
      */ 
      const response = await axios.post(`/productEdit/`, formData);
  ...
  const productDelete = async (e, id) => {
    ...
    const response = await axios.post(`/productDelete/${id}`);
  ...
  return (
    ...
    <h3>Laravel 10 Flask (Route/Api) and React.js 18</h3>
